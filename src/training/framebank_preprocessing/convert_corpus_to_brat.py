...
'''
    ...
    roles = str()
    for _, role_id in sorted(role_index.iteritems(), key = lambda e: e[0]):
...
'''
    bad_roles = list()
    roles = unicode()
    for role_str, role_id in role_index.iteritems():
...
def convert(input_file_path, output_dir_path, converter, logger):
    ...
    for num, (pred, ex_ids) in enumerate(sorted(verb_example_index.iteritems(), 
                                                key = lambda e: e[0])):
